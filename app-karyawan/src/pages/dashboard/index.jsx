				...
				<Grid container spacing={2.5}>
					...
					<Grid item xs={12} md={4}>
						<Card sx={{ p: 2.5 }}>
							...
							<Chart options={siteChartOptions} series={siteChartSeries} type="bar" height={280} />
						</Card>
					</Grid>
				</Grid>
										...
										<TableBody>
											{tables.recentLeaves.map((item) => (
												<TableRow key={item.id}>
													...
													<TableCell>
														<Chip
															label={LEAVE_STATUS_MAP[item.status]?.label || item.status}
															size="small"
															color={LEAVE_STATUS_MAP[item.status]?.color || 'default'}
														/>
													</TableCell>
												</TableRow>
											))}
